...
void loop() {
    ...
    int motionState = readMotionSensor();

    if (motionState == HIGH) {
        // Encender el LED y activar el zumbador cuando se detecte movimiento
        digitalWrite(LED_PIN, HIGH);
        tone(BUZZER_PIN, 1000); // Activar el zumbador a una frecuencia de 1000 Hz
        Serial.println("¡Se detectó movimiento!");
    } else {
        // Apagar el LED y detener el zumbador cuando no haya movimiento
